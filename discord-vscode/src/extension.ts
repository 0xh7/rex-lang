...
import { CLIENT_ID, REX_CLIENT_ID, CONFIG_KEYS } from './constants';
...
function getCurrentClientId(): string {
	...
	if (editor?.document.languageId === 'rex') {
		return REX_CLIENT_ID;
	}

	return CLIENT_ID;
}

let currentClientId = getCurrentClientId();
let rpc = new Client({ transport: { type: 'ipc' }, clientId: currentClientId });
...
async function login() {
	...
	rpc = new Client({ transport: { type: 'ipc' }, clientId: currentClientId });
	...
	rpc.on('ready', () => {
		...
		void sendActivity();
		const onChangeActiveTextEditor = window.onDidChangeActiveTextEditor(async () => {
			const newClientId = getCurrentClientId();
			if (newClientId !== currentClientId) {
				log(LogLevel.Info, `Switching Discord Application ID from ${currentClientId} to ${newClientId}`);
				cleanUp();
				await rpc.destroy();
				await login();
			} else {
